...
def doubletsReadPostMod(detIn,detOu,modIn,modOu,datasets,train=False,
  ...
  trueClusters = numpy.array(numpy.extract(truesIndex, train_clusters))
  trueLabels   = numpy.extract(truesIndex, train_clusters_labels)
  ...
  idxs = numpy.random.randint(falseClusters.shape[0], size=1000)
  ...
  truesIndex  =  (test_clusters_labels[:,1]==1.0)
  falsesIndex =  (test_clusters_labels[:,1]==0.0)
