...
def datafiltering(filters,alldata,savetoh=False,shuffle=False,sanitize=False,sanratio=1.0):
            ...
            if k in ranglabs:
                if isinstance(r,(list,tuple)):
                    if all(isinstance(x,(int,float)) for x in r) and len(r)==2:
                        ...
                        alldata = alldata[alldata[:,datadict[k]]<max(r)]
                        alldata = alldata[alldata[:,datadict[k]]<min(r)]
                        print(str(oldsize-alldata.shape[0]) + " doublets dropped.")
                        oldsize = alldata.shape[0]
                    else:
...
def clustersInput(alldata,cols=8,rows=8,infos=61,dropEdge=False,dropBad=True,dropBig=False,dropCosmic=False,dropCharge=False):
