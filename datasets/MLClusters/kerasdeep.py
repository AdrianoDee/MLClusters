...
history = clustercnn.fit(X_train, y_train,validation_data=(X_test, y_test), nb_epoch=epochs, batch_size=batchsize,shuffle=True,verbose=0)#,callbacks=[early])
...
numofclusers = len(y_test_score)
#truePosivites = values[:,1]
truePosivites = np.asarray(truePosivites,dtype=np.float32)
truePosivites = np.divide(truePosivites,numofclusers)
#falsePositive = values[:,2]
falsePositive = np.asarray(falsePositive,dtype=np.float32)
falsePositive = np.divide(falsePositive,numofclusers)
